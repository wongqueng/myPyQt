...
class ImageWidget(QtGui.QWidget):
    ...
    def paintAsThumb(self, painter):
        name_height = min(self.height() * 0.15, 20)
        ...
        # 脚标字符
        painter.drawText(self.edge_size, name_ty, self.displayText)
    ...
    def dirTreeClicked(self):
        ...
        for item in os.listdir(pathSelected):
            if item.split('.')[-1] in FILE_TYPE:
                ...
                widget = ImageWidget()
                widget.displayText = unicode(item.decode("gb2312"))
                widget.setThumb((unicode(pathSelected+ '/' + item.decode("gb2312"))))
                self.imageContainer.addWidget(widget)  # self.imageContainer.layout()
